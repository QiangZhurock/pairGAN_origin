...
def get_norm_layer(norm_type='instance'):
    if norm_type == 'batch':
        norm_layer = functools.partial(nn.BatchNorm2d, affine=True) #https://blog.csdn.net/cassiepython/article/details/76653897
...
def define_G(input_nc, output_nc, ngf, which_model_netG, norm='batch', use_dropout=False, init_type='normal', gpu_ids=[]):
    netG = None
    norm_layer = get_norm_layer(norm_type=norm)#define the norm_layer, opt.norm = instance
...
class SRNGenerator(nn.Module):
    def      __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        ...
        model = [nn.ReflectionPad2d(3),
                 nn.Conv2d(input_nc, 16, kernel_size=7, padding=0,
                           bias=use_bias),
                 norm_layer(16),
        ...
        #     mult = 2**i
        #     model += [nn.Conv2d(16* mult, 16 * mult * 2, kernel_size=3,
        #                         stride=2, padding=1, bias=use_bias),
        #               norm_layer(ngf * mult * 2),
        #               nn.ReLU(True)]

        model += [nn.Conv2d(16, 32, kernel_size=3,
                            stride=2, padding=1, bias=use_bias),
                  norm_layer(32),
        ...
        #     #transpose:https://ptorch.com/docs/1/torch-nn
        ...
        DeConvModel = [nn.ConvTranspose2d(512, 256,
                                     kernel_size=3, stride=2,
                                     padding=1, output_padding=1,
                                     bias=use_bias),
                  norm_layer(256),
        ...
        DeConvModel += [nn.ConvTranspose2d(256, 128,
                                     kernel_size=3, stride=2,
                                     padding=1, output_padding=1,
                                     bias=use_bias),
                  norm_layer(128),
        ...
        DeConvModel += [nn.ConvTranspose2d(128, 64,
                                     kernel_size=3, stride=2,
                                     padding=1, output_padding=1,
                                     bias=use_bias),
                  norm_layer(64),
        ...
        DeConvModel += [nn.ConvTranspose2d(64, 32,
                                     kernel_size=3, stride=2,
                                     padding=1, output_padding=1,
                                     bias=use_bias),
                  norm_layer(32),
        ...
        DeConvModel += [nn.ConvTranspose2d(32, 16,
                                     kernel_size=3, stride=2,
                                     padding=1, output_padding=1,
                                     bias=use_bias),
                  norm_layer(16),
